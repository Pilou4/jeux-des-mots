...
function checkPlayedWords()
{
	...
	let tileItems =	touched.flat().filter(e => e.witness != "").map(e => {return {code: e.code + ',' + e.word, position: e.position};});
	tileItems.forEach(e =>
		{
	...
	tileItems.forEach(e =>
		{
			...
			gamePoints.set(e.code, (wordPoints * (wordBonus == 0 ? 1 : wordBonus)) + extraPoints);
		}
	);
