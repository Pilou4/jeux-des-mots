function drawBoard()
{
	...
	for (let line = -1; line < 16; line++)
	{
		...
		for (let column = -1; column < 16; column++)
		{
			...
			{
				cells += '<div class="cell" data-coords="' + line + "-" + column + '">' + (specialCell ? '<div class="specialCell ' + specialCell.type + '">' + (line == 7 && column == 7 ? '<img class="star" src="images/star.png">' : '<label class="specialCellText">' + specialCell.text + '</label>') + '</div>' : '') + '</div>';
			}
		}
	}
...
function checkPositioning()
{
	...
	if ($('.playedTileItem').length == 0)
	{
		if (word.length < 2)
		{
	 	...
	 	if (!letters.some(e => parseInt($(e).offset().left / (board.tileSize + board.offset)) == 7 && parseInt($(e).offset().top / (board.tileSize + board.offset)) - 1 == 7))
	 	{
...
{
	$('body').append('<div class="screenMask"></div><div class="jokerScreen"><label class="jokerTitle">Choisissez la lettre à utiliser</label><div class="jokerBox"></div></div>');
	for (let t = 65; t < 91; t++)
	{
...
function showGameStats()
{
	...
	{
		currentGame.players.forEach((e, i) => $('.scoreZone').append('<fieldset class="scoreField"><label class="playerName">' + (i + 1) + ' - ' + e.name + '</label><label class="scoreValue">' + e.score + '</label></fieldset>'));
	}
}
