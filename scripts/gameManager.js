...
function updateTurn ()
{
	...
	{
		if (skippedTurns >= 2 * currentGame.players.length || (currentPlayer.pid == 1 && currentGame.lastTurn))
		{
			endGame();
		}
		else
	...
	players.forEach(e => 
		{
			if (e.rack != '')
			{
				const points = e.rack.split('').reduce((p, c) => p + lettersDistribution.find(g => g.letter == c).value, 0);
				e.score -= points;
...
function endGame()
{
	...
	if (currentGame.players.find(e => e.uid == currentGame.turnId).pid == currentPlayer.pid)
	{
		currentGame.ranking = ranking;
		currentGame.turnId = currentPlayer.pid == currentGame.players.length ? currentGame.players.find(e => e.pid == 1).uid : currentGame.players.find(e => e.pid == currentPlayer.pid + 1).uid;
	}
