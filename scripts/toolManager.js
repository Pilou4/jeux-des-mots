function validate()
{
	if (checkValidity(playedWords.words))
	{
		...
		$('.historyZone').html(('000' + ($('.historyZone').html().split('<br>').length)).substr(-3) + ' - ' + playedWords.words.join(' + ') + ' : +' + playedWords.score + ' - ' + currentPlayer.name + '<br>' + $('.historyZone').html());
...
{
	const oldLetters = $('.playerRack .tileItem').get().map(e => {const letter = $(e).find('label:first').text(); return letter == '' ? '*' : letter;}).join('');
	...
	prepareTiles();
	players[0].score -= 15;
	$('.historyZone').html(('000' + ($('.historyZone').html().split('<br>').length)).substr(-3) + ' - Echange : -15 - (' + currentPlayer.name + ')<br>' + $('.historyZone').html());
	updateGame();
	saveGames();
}
...
function showGames()
{
	...
	games.forEach((e, i) =>
		{
			$('.continueGame ul:first').append('<li><div class="gameDiv"><label class="gameName">' + e.name + '</label><label class="gamePlayerNames">' + e.players.length + ' ' + (e.players.length > 1 ? 'joueurs : ' : 'joueur : ') + e.players.map(f => {return f.uid != e.turnId ? f.name : '<span class="currentPlayer">' + f.name + '</span>';}).join(' - ') + '</label><img class="vsButton" id="loadGame" data-index="' + i + '" src="images/validate.png"><img class="vsButton" id="deleteGame" data-index="' + i + '" src="images/resign.png"></div></li>') ;
		}
	);
}
...
function checkCode(mode)
{
	...
	else
	{
		...
		const keepMapping = testGame ? testGame.mapping : decodedGame.players[0].uid;
...
function loadGame(e)
{
	...
	if (currentGame.mode == "distant" && user && user.uid != currentPlayer.uid && !currentGame.terminated)
	{
		...
		$('.gameFromCodeScreen #continueGame').on('click', e =>
			{
				checkCode('continueGame');
			}
		);
		...
		currentGame.board.match(/.{4}/g).forEach(f =>
			{
				...
				$('.gameArea').append('<div class="playedTileItem" data-coords="' + line + '-' + column + '" style="position: absolute; left: ' + (board.xOffset + (column * (board.tileSize + board.offset))) + 'px; top: ' + (board.yOffset + (line  * (board.tileSize + board.offset))) + 'px;"><label class="tileLetter' + (value == 0 ? " jokerTile" : "") + '">' + (letter == '*' ? '' : letter) + '</label><label class="tilePoints">' + value + '</label></div>');
			}
		);
	}
